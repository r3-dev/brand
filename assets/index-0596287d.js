...
`;class i extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
      <style>${l}</style>
      <a target="_blank">${c}</a>
    `}connectedCallback(){if(!this.isShowBadge())return;const a=this.shadowRoot.querySelector("a");a.href=this.getAttribute("href")??"#",a.classList.add("visible")}isShowBadge(){const a=new URLSearchParams(location.search),n=this.getAttribute("key")??"utm_source",r=this.getAttribute("value")??"upwork",e=a.get(n);return!e||!r?!1:e===r}}customElements.define("r3-badge",i);
